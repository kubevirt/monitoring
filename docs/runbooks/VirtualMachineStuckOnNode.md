...
kubectl logs <virt-handler-pod> -n kubevirt

# Alternative: Check resource usage via pod metrics
...
```bash
# Check if container images can be pulled on the node
kubectl debug node/<node-name> -it --image=busybox
# Then inside the debug pod, try pulling the image:
...
# Alternative: Check which container runtime is in use
...
1. **Clear image cache** on the problematic node:
   ```bash
   # SSH to the node or use kubectl debug to run on the node:
...
2. **Restart node-level components**:
   ...
   kubectl delete pod <virt-handler-pod> -n kubevirt
   ```
...
- Suspected KubeVirt component bugs or regressions
